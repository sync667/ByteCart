...
public class ByteCartUpdaterMoveListener implements Listener{
    ...
    public void onVehicleMoveEvent(VehicleMoveEvent event) {
        ...
        if (v instanceof InventoryHolder) {
            ...
            if (CraftlandiaRails.myPlugin.getWandererManager().isWanderer(inv, "Updater")) {
                Bukkit.getServer().getPluginManager().callEvent(new UpdaterMoveEvent(event));
                return;
            }
        }

        if (CraftlandiaRails.myPlugin.getWandererManager().getFactory("Updater").areAllRemoved()) {
            removeListener();
        }
    }
    ...
    public void onVehicleDestroy(VehicleDestroyEvent event) {
        ...
        if (v instanceof InventoryHolder) {
            Inventory inv = ((InventoryHolder) v).getInventory();
            if (CraftlandiaRails.myPlugin.getWandererManager().isWanderer(inv, "Updater")) {
                CraftlandiaRails.myPlugin.getWandererManager().getFactory("Updater").destroyWanderer(inv);
