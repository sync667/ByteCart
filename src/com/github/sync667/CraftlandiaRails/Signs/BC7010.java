...
public class BC7010 extends AbstractTriggeredSign implements Triggable, Clickable{
     ...
     */
    BC7010(org.bukkit.block.Block block, org.bukkit.entity.Vehicle vehicle) {
        super(block, vehicle);
    }
    ...
    public final void trigger() {
		...
		if (! this.isHolderAllowed() || CraftlandiaRails.myPlugin.getWandererManager().isWanderer(getInventory())) {
			return;
		}
    ...
    /**
     * Spawn a ticket in inventory and set the destination address The train bit is not set.
     *
        ...
        if (IPaddress == null || ! IPaddress.setAddress(SignAddress)) {
            ...
            if (this.getInventory().getHolder() instanceof Player) {
                ((Player) this.getInventory().getHolder()).sendMessage(
                        ChatColor.GREEN + "[CraftlandiaRails] " + ChatColor.RED +
    ...
    protected void infoPlayer(String signAddress) {
        ((Player) this.getInventory().getHolder()).sendMessage(
                ChatColor.DARK_GREEN + "[CraftlandiaRails] " + ChatColor.YELLOW +
                        ...
                        signAddress);
		if (this.getVehicle() == null && ! CraftlandiaRails.myPlugin.getConfig().getBoolean("usebooks")) {
			((Player) this.getInventory().getHolder()).sendMessage(
					ChatColor.DARK_GREEN + "[CraftlandiaRails] " + ChatColor.YELLOW +
							CraftlandiaRails.myPlugin.getConfig().getString("Info.SetAddress2"));
		}
    }
