...
class BC7014 extends BC7010 implements Triggable{
    ...
    BC7014(org.bukkit.block.Block block, org.bukkit.entity.Vehicle vehicle) {
        super(block, vehicle);
    ...
    protected Address getAddressToWrite() {
        ...
        if (CraftlandiaRails.debug) {
            CraftlandiaRails.log.info("CraftlandiaRails: " + this.getName() + " wire : " + wire.getAmount());
        }
    ...
    protected String format(RegistryInput wire, AddressRouted InvAddress) {
        return "" + InvAddress.getRegion().getAmount() + "." + InvAddress.getTrack().getAmount() + "." +
                wire.getAmount();
    }
