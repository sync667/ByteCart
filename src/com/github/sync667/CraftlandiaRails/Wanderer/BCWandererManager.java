...
public class BCWandererManager implements WandererManager{
    ...
    public WandererFactory getFactory(Inventory inv) throws ClassNotFoundException, IOException {
		if (isWanderer(inv) && getWandererContent(inv) != null) {
    ...
    private boolean isWanderer(Inventory inv, Scope scope, String suffix, String type) {
        ...
        if (stack != null && stack.getType().equals(Material.WRITTEN_BOOK) && stack.hasItemMeta()) {
            ...
            match.append(".*");
            if (InventoryFile.isInventoryFile(inv, type) && booktitle.matches(match.toString())) {
                return true;
            }
        }
        return false;
    ...
    @Override
    public WandererContent getWandererContent(Inventory inv) throws IOException, ClassNotFoundException {
    ...
    public void saveContent(InventoryContent rte, String suffix, Level level)
        ...
        if (creation != expiration && Calendar.getInstance().getTimeInMillis() > expiration) {
            LogUtil.sendSuccess(rte.getPlayer(),
                    "CraftlandiaRails : " + suffix + " created " + (new Date(rte.getCreationtime())).toString() +
                            " expired");
        ...
        try {
            ...
            getFactory(inv).destroyWanderer(inv);
            CraftlandiaRails.log.info("CraftlandiaRails: I/O error (maximum capacity reached), updater deleted");
        }
    }

    private static InventoryFile createWanderer(Inventory inv, int region, Level level, Player player, String name,
                                                Level type) throws IOException {
