...
final class BCBukkitRunnable <K>{
    ...
    BukkitTask renewTaskLater(long duration, Object... objects) {
        ...
        synchronized (map) {
            if (! Expirable.getThreadMap().containsKey(Key)) {
                ...
                if (old.isSync()) {
                    task = runnable.runTaskLater(CraftlandiaRails.myPlugin, duration);
                } else {
                    task = runnable.runTaskLaterAsynchronously(CraftlandiaRails.myPlugin, duration);
                }
                old.cancel();
            }
    ...
    private BukkitTask runTaskLater(Object... objects) {
        ...
        org.bukkit.scheduler.BukkitTask task =
                runnable.runTaskLater(CraftlandiaRails.myPlugin, Expirable.getDuration());
        return task;
    }
    ...
    private BukkitTask runTaskLaterAsynchronously(Object... objects) {
        BukkitRunnable runnable = new Expire(Expirable, Key, objects);
        org.bukkit.scheduler.BukkitTask task =
                runnable.runTaskLaterAsynchronously(CraftlandiaRails.myPlugin, Expirable.getDuration());
        return task;
    }
