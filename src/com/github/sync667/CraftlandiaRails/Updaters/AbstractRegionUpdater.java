...
abstract class AbstractRegionUpdater extends DefaultRouterWanderer{
        ...
        if (isRouteConsumer()) {
            ...
            if (current >= 0 && (! connected.contains(current) || connected.size() != 1)) {
                ...
                if (CraftlandiaRails.debug) {
                    CraftlandiaRails.log.info("CraftlandiaRails : Wanderer : storing ring " + current + " direction " +
                            getFrom().ToString());
                }

                getRoutingTable().setEntry(current, getFrom().getBlockFace(), 0);
                CraftlandiaRails.myPlugin.getWandererManager().getFactory("Updater").updateTimestamp(Routes);
            ...
            //			CraftlandiaRails.myPlugin.getUm().getMapRoutes().put(getVehicle().getEntityId(), getRoutes(), false);
    ...
    /* (non-Javadoc)
     * @see com.github.catageek.CraftlandiaRails.Routing.DefaultRouterWanderer#doAction(org.bukkit.block.BlockFace)
     */
    @Override
    public void doAction(BlockFace To) {
        ...
        current = (current == - 2 ? 0 : current);
        if (CraftlandiaRails.debug) {
            CraftlandiaRails.log.info("CraftlandiaRails : doAction() : current is " + current);
        }
        ...
        try {
            CraftlandiaRails.myPlugin.getWandererManager().saveContent(Routes, "Updater", this.getLevel());
    ...
    /* (non-Javadoc)
     * @see com.github.catageek.CraftlandiaRails.Routing.DefaultRouterWanderer#giveRouterDirection()
     */
    @Override
