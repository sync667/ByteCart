...
public final class UpdaterFactory implements WandererFactory{
    ...
    public Wanderer getWanderer(BCSign bc, Inventory inv) throws ClassNotFoundException, IOException {
        ...
        if (wm.isWanderer(inv, Level.REGION, "Updater") && (rte = getUpdaterContent(inv)) != null) {
            return new UpdaterRegion(bc, rte);
        }

        if (wm.isWanderer(inv, Level.RESET_REGION, "Updater") && (rte = getUpdaterContent(inv)) != null) {
            return new UpdaterResetRegion(bc, rte);
        }

        if (wm.isWanderer(inv, Level.BACKBONE, "Updater") && (rte = getUpdaterContent(inv)) != null) {
            return new UpdaterBackBone(bc, rte);
        }

        if (wm.isWanderer(inv, Level.RESET_BACKBONE, "Updater") && (rte = getUpdaterContent(inv)) != null) {
            return new UpdaterResetBackbone(bc, rte);
        }

        if (wm.isWanderer(inv, Level.RESET_LOCAL, "Updater") && (rte = getUpdaterContent(inv)) != null) {
            return new UpdaterResetLocal(bc, rte);
        }

        if (wm.isWanderer(inv, Level.LOCAL, "Updater") && (rte = getUpdaterContent(inv)) != null) {
    ...
    @Override
    public void createWanderer(int id, Inventory inv, int region, Level level, Player player, boolean isfullreset,
                               boolean isnew) {
        UpdaterContent rte;
        ...
        try {
            CraftlandiaRails.myPlugin.getWandererManager().saveContent(rte, "Updater", level);
        } catch (ClassNotFoundException | IOException e) {
            // TODO Auto-generated catch block
            e.printStackTrace();
        }
        updaterset.getMap().add(id);
        LogUtil.sendError(player, CraftlandiaRails.myPlugin.getConfig().getString("Info.SetUpdater"));
    }
