...
public final class BCDynmapPlugin implements Listener{
    ...
    private static final String name =
            CraftlandiaRails.myPlugin.getConfig().getString("dynmap.layer", "CraftlandiaRails");
    ...
    public BCDynmapPlugin() {
        ...
        try {
            ...
            throw new MissingResourceException(
                    "Dynmap plugin is not present, please install it or set dynmap = false in config file",
                    MarkerAPI.class.getName(), "dynmap");
        }
        ...
        if (defaulticon == null) {
            throw new MissingResourceException("Station icon could not be loaded", MarkerAPI.class.getName(),
                    "station");
        }
        ...
        if (updatericon == null) {
            throw new MissingResourceException("Updater icon could not be loaded", MarkerAPI.class.getName(),
                    "updater");
        }
    ...
    /**
     * Load the CraftlandiaRails icon
     *
    ...
    private MarkerIcon loadIcon(MarkerAPI markerapi, String id, String label, String file) {
        ...
        if (icon == null) {
            try (InputStream png = getClass().getResourceAsStream(file)) {
                ...
                throw new MissingResourceException("CraftlandiaRails : icon could not be loaded",
    ...
    public void onUpdaterPassStation(UpdaterPassStationEvent event) {
        ...
        if (marker != null) {
            if (address.isValid() &&
                    checkMarkerInformation(marker, block, address.toString(), event.getIc().getFriendlyName())) {
                return;
            }
            marker.deleteMarker();
        }
    ...
    private static void addMarker(String label, Location loc, MarkerIcon markericon) {
        addMarker(loc.toString(), label, loc, markericon);
    }
    ...
    private static void addMarker(String key, String label, Location loc, MarkerIcon markericon) {
        markerset.createMarker(key, label, loc.getWorld().getName(), loc.getX(), loc.getY(), loc.getZ(), markericon,
                true);
    }
    ...
    /**
     * Search for stations that do not exist anymore and remove corresponding markers.
     */
    public static void removeObsoleteMarkers() {
        if (markerset != null) {
            Bukkit.getScheduler().runTask(CraftlandiaRails.myPlugin, new searchObsoleteMarkers());
        }
    }
}
