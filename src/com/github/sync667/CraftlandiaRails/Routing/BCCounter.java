...
public final class BCCounter implements Serializable, Counter{
    ...
    public int getMinimum(RoutingTableWritable routes, DirectionRegistry from) {
        ...
        while (it.hasNext()) {
            ...
            int value;
            if ((value = map.get(ring)) < min && routes.getDirection(ring) != from.getBlockFace() && ring != 0) {
                min = value;
                index = ring;
            }
        }
        if (CraftlandiaRails.debug) {
            CraftlandiaRails.log.info("CraftlandiaRails : minimum found ring " + index + " with " + min);
        }
        return index;
    ...
    public final String toString() {
        ...
        while (it.hasNext()) {
            ...
            s += "CraftlandiaRails: Count for ring " + ring + " = " + count + "\n";
        }
        return s;
    }
